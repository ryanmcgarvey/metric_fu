module MetricFu
  ...
  class PerfGchartGrapher < PerfGrapher
    def graph!
      all_values = []
      @test_runs_list.each_pair do |test,scores|
        all_values.push(scores)
      end
      determine_y_axis_scale(all_values.flatten.uniq)
      url = Gchart.line(
              :size => GCHART_GRAPH_SIZE,
              :title => URI.escape("Performance Benchmarks"),
              :data => all_values,
              :bar_colors => COLORS[0..@test_runs_list.size-1],
              :legend => @test_runs_list.keys,
              :custom => "chdlp=t",
              :max_value => @max_value,
              :axis_with_labels => 'x,y',
              :axis_labels => [@labels.values, @yaxis],
              :format => 'file',
              :filename => File.join(MetricFu.output_directory, 'perf.png'))
    end
  end
